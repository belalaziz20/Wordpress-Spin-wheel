(function () {
  console.log('Spin Wheel JS Loaded');//ss
  ...
  const SpinWheel = {
    ...
    createImages: function () {
      ...
      // );
      if (WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImage && WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImage !== 'none') {
        this.imgElement = this.createImageElement(
          "spin-wheel-img",
          WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImage
        );
      } else {
        this.imgElement = this.createImageElement(
          "spin-wheel-img",
      ...
      if (WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImageOverlay) {
        this.imgOverlayElement = this.createImageElement(
          "spin-wheel-img-overlay",
          WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImageOverlay
        );
        // console.log(WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImageOverlay);
      } else {
        this.imgOverlayElement = this.createImageElement(
          "spin-wheel-img-overlay",
          WOF_LocalizeConfig.assetsUrl + "imgs/example-0-overlay.svg"
    ...
    bindEvents: function () {
      document.addEventListener("click", (e) => {
        // Check if the clicked element is the "Get Lucky" button
        if (e.target.matches("#spin-wheel-spin-btn")) {
          ...
          const campaign =
            WOF_LocalizeConfig?.wheel_data?.frontEndVisibility?.campaign_name;
          ...
          if (name && email) {
            ...
            // Show the loader
            document
              .querySelector(".spin-wheel-form-loader")
              .classList.remove("spin-wheel-hidden");

            // Set cookies for form data
            ...
            // Trigger the spin wheel click action
            this.handleSpinClick();
          } else {
            // Handle case where name or email is missing
    ...
    getProps: function () {
      ...
      if (WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.centerImage == 'none') {
        delete props.image;
      }
    ...
    confettiEffect: function () {
      if (!WOF_LocalizeConfig?.wheel_data?.wheelStyles?.othersData?.confettiEffects) {
        return;
      }
    ...
    managePopups: function (data) {
      if (true == data?.win) {
        ...
        winDom.querySelector(".spin-wheel-win-coupon-wrap button").addEventListener("click", () => {
          ...
          if (winInfo?.buttonTextCopied) {
            winDom.querySelector(".spin-wheel-win-coupon-wrap button").innerHTML = winInfo.buttonTextCopied;
          } else {
    ...
    setStyles: function () {
      ...
      if (submitForm) {
        ...
        if (submitForm.bodyBg) {
          document.querySelector(".spin-wheel-spin-wrapper").style.background = submitForm.bodyBg;
        }
        if (submitForm.title) {
        ...
        if (submitForm.buttonText) {
          $button.innerHTML = submitForm.buttonText;
        }
      ...
      if (winInfo) {
        ...
        if (winInfo.buttonText) {
          $button.innerHTML = winInfo.buttonText;
        }
